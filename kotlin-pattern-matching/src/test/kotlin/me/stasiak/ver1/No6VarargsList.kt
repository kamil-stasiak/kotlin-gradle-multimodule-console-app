...
class No6VarargsList {
    ...
    fun `should match third, shorter (Dog)`() {
        val match = match(secretCombination) {
            case(5, 5, 5, 5).then { "Tiger" }
            case(5, 2, 0, 5).then { "Horse" }
            case(5, 2, 1).then { "Dog" }
            case().then { "Other" }
        }
    ...
    class Matcher(private val obj: List<Int>) {
        ...
        fun case(vararg predicate: Int): Case {
            return Case(this, predicate.toList())
        }
